...
xgen_gdl90:
	...
	go build $(BUILDINFO) -p 4 main/gen_gdl90.go main/traffic.go main/gps.go main/network.go main/managementinterface.go main/sdr.go main/uibroadcast.go main/monotonic.go main/datalog.go main/equations.go main/ahrs.go

xrtimulib:
	git submodule update --init
	cd RTIMULib/RTIMULib && mkdir build
	cd build && cmake ..
